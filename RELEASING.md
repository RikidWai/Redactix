...
The normalized PyPI name `redactix` is already occupied by an existing `Redactix` project, so this package is published under the distribution name `pyredactix` while keeping the Python import name `redactix`.

Publish and document installation as `pip install pyredactix`. The Python import remains `import redactix`.
...
python -m pip install --index-url https://test.pypi.org/simple/ --extra-index-url https://pypi.org/simple/ pyredactix
